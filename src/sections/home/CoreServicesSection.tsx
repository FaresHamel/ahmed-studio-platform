    ...
    <>
      <section className="relative w-full min-h-[800px] py-20 md:py-28 overflow-hidden flex flex-col justify-around my-10">
        <div className="absolute inset-0 z-0">
          <Image src="/images/videoTool.jpg" alt="Our Services Background" fill priority className="object-cover object-center scale-105" sizes="100vw" />
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto px-4">
          <div className="text-center mb-24 md:mb-16">
            <h2 className="text-4xl md:text-5xl font-bold tracking-wide text-white font-serif drop-shadow-md">{cs.title}</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-0 gap-y-0 text-center relative">
            ...
            {pillars.map((pillar, index) => (
              <div key={pillar.id} className={`flex flex-col items-center px-4 py-8 md:p-6 text-center group relative ${index % 2 === 0 ? "border-r border-white/20 md:border-r-0" : ""} ${index !== 0 ? "md:border-l md:border-white/20" : ""}`}>
                ...
                <h3 className="text-base md:text-xl font-[400] tracking-wide mb-6 text-stone-200 min-h-[48px] flex items-center justify-center">{pillar.title}</h3>
                <button onClick={() => { setActiveTab(pillar.id); setIsOpen(true); }} className="w-10 h-10 rounded-full bg-[#ebdcd0] hover:bg-white text-[#112438] flex items-center justify-center transition-all duration-300 shadow-md active:scale-90 mt-auto" aria-label="Open details modal">
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10 bg-black/70 backdrop-blur-sm transition-opacity duration-300 animate-fadeIn">
          <div className="relative w-full max-w-5xl bg-[#3c241c] rounded-2xl overflow-hidden shadow-2xl border border-stone-800 flex flex-col md:flex-row-reverse text-left" dir="ltr">
            <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 z-30 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-stone-300 hover:text-white flex items-center justify-center transition-all" aria-label="Close modal">
            ...
            <div className="w-full md:w-[32%] bg-[#2e1a14] p-6 flex flex-col gap-3 justify-center border-b md:border-b-0 md:border-r border-stone-800/40 shrink-0">
              <span className="text-amber-100/40 text-xs font-bold tracking-wider mb-2 block px-2">{cs.ourCommunity}</span>
              {services.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.category)} className={`w-full text-left px-4 py-3 rounded-lg text-sm md:text-base font-semibold transition-all duration-200 ${activeTab === tab.category ? "bg-[#ebdcd0] text-[#3c241c] shadow-md transform translate-x-1" : "bg-[#ebdcd0]/10 text-stone-300 hover:bg-[#ebdcd0]/20"}`}>
                  {tab.title}
                </button>
              ))}
            </div>
            ...
            <div className="w-full md:w-[68%] p-6 md:p-8 flex flex-col justify-between">
              <div className="relative w-full aspect-[16/9] rounded-xl overflow-hidden shadow-md mb-6 border border-stone-800/50">
                <Image src={currentData.image} alt={currentData.title} fill className="object-cover transition-transform duration-500 hover:scale-105" sizes="(max-width: 768px) 100vw, 60vw" />
              </div>
              <div className="flex-1 flex flex-col justify-end">
                <h2 className="text-2xl md:text-3xl font-bold text-amber-100 mb-3">{currentData.title}</h2>
                <p className="text-stone-300 text-sm md:text-base leading-relaxed mb-4 font-light">{currentData.description}</p>
